...
export const Operate: React.FC = () => {
  ...
  const originData = (zdStore: IZdyzb, zdyStore: IZdyzbFzs[]) => {
    ...
    originData.push(
      ...
      },
      {
        zbName: '应收账款周转天数（天）',
        key: 'yszkzzts',
        bbz: zdStore.yszkzzts,
        normal: zdyStore.map(item => {
          if (item.zbKey === 'yszkzzts') {
            return item.normal
          }
        }),
        unnormal: zdyStore.map(item => {
  ...
  const UpdateForm = (record: Item) => {
    console.log(record.zbName)
    console.log(record.bbz)
    return (
      <Modal
